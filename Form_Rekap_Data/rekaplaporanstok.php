...
$stk = mysqli_query($conn, "SELECT * FROM db_laporanstok");

// tampilkan seluruh data sesuai dengan halaman
$formlaporan = query("SELECT * FROM db_laporanstok ORDER BY id DESC LIMIT $DataAwal, $jumlahdataperhalaman");

// tombol cari ditekan
if (isset($_POST["caristok"])) {
  $stk = caristok($_POST["keyword"]);
}
...
if (isset($_POST['submit'])) {
  ...
  if (!empty($date1) && !empty($date2)) {
    ...
    $stk = mysqli_query($conn, "SELECT * FROM db_laporanstok WHERE tanggal BETWEEN '$date1' and '$date2'");
  } else {
...
<head>
  ...
  <title>Rekap laporan stok bahan makanan</title>
...
<body>
  ...
  <div class="container mt-2">
    ...
    <!-- Tittle -->
    <h1 style="text-align: center; margin-bottom:10px;">Rekap laporan Stok Bahan Makanan</h1>
    <form action="" method="post" class="mb-2">
      <input type="text" name="keyword" size="40" autofocus placeholder="masukkan keyword pencarian!!" autocomplete="off">
      <button type="submit" name="caristok">Cari Data</button>
    </form>
    <form action="" method="post" class="mb-2">
      <div class="row g-2 align-items-center mb-0">
        ...
        <div class="col-auto ">
          <a href="../Laporan_Stok/laporanStok.php" class="btn btn-dark"> Form Input</a>
        </div>
          ...
          <tr class="text-center" style="font-size:15px">
            ...
            <td>
              <button type="button" class="btn btn-info"> 
                  <a href="../editdata/editlaporan.php?id=<?= $row["id"]; ?>" onclick="return confirm('yakin?')" style="text-decoration: none; color:black"><span class="bi bi-pencil-square">Edit</span></a>
              </button>
              <button type="button" class="btn btn-warning"> 
                  <a href="../Function_Hapus/hapuslaporan.php?id=<?= $row["id"]; ?>" onclick="return confirm('yakin?')" style="text-decoration: none; color:black"><span class="bi bi-trash">Hapus</span></a>
              </button>
            </td>
          </tr>
        <?php $i++;
    ...
    <nav>
      ...
      <div class="d-flex justify-content-end mb-2">
        ...
        <a target="_blank" href="../cetak_laporanstok.php?TglAwal=<?= $date1 ?>&TglAkhir=<?= $date2 ?>"><button style="background-color: smoke;">Download Data</button></a>
      </div>
    </nav>
    <div class="footer">
      <p><?= $Credit ?></p>
