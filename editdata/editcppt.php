...
if (!isset($_GET["id"])) {
  ...
  $data_cppt = query("SELECT * FROM db_cppt WHERE id = $cppt");

  $TotalData = count($data_cppt);

  if ($TotalData == 0) {
    echo "ID tidak ditemukan";
    exit;
  } else {
    $data_cppt = query("SELECT * FROM db_cppt WHERE id = $cppt")[0];
  }
}
...
<body>
  ...
  <div class="container mt-1">
    <h4 class="text-center">Edit Catatan Perkembangan Pasien</h4>
    <div style="background-color: rgb(228, 228, 228)">
      <div class="container-fluid">
        <form action="" method="post" class="row g-3 mt-2">
        <input type="hidden" name="id" value="<?= $data_cppt["id"]; ?>">
          <div class="col-md-3">
            <label for="NoRm" class="form-label">No RM</label>
            <input type="text" name="NoRm" class="form-control form-control-sm" id="NoRm"  value="<?= $data_cppt["no_rm"]; ?>"/>
          </div>
          <div class="col-md-3">
            <label for="tgl" class="form-label">Tanggal</label>
            <input type="date" name="tgl" class="form-control form-control-sm" id="tgl" value="<?= $data_cppt["tanggal"]; ?>"/>
          </div>
          <div class="col-md-3">
            <label for="NamaPasien" class="form-label">Nama Pasien</label>
            <input type="text" name="NamaPasien" class="form-control form-control-sm" id="NamaPasien" value="<?= $data_cppt["nama_pasien"]; ?>" />
          </div>
          ...
          <div class="col-md-3">
            ...
            <select name="Ruangan" id="Ruangan" class="form-select form-select-sm" required>
              ...
              while ($data = mysqli_fetch_array($sql)) {
                echo "<option " . ($data["nama_ruangan"] == $data_cppt["nama_ruangan"] ? "selected" : "") . " value='$data[nama_ruangan]'>$data[nama_ruangan]</option>";
              }
              ?>
            </select>
          </div>
          <div class="col-md-3">
            ...
            <input type="text" name="ptinput" class="form-control form-control-sm" id="ptinput" value="<?= $data_cppt["petugas"]; ?>" />
          </div>
          <div class="col-md-3">
            <label for="Diagnosa" class="form-label">Diagnosa</label>
            <input type="text" name="Diagnosa" class="form-control form-control-sm" id="Diagnosa" value="<?= $data_cppt["diagnosa"]; ?>" />
          </div>
          <div class="col-md-3">
            <label for="Kamar" class="form-label">No_Kamar</label>
            <input type="text" name="Kamar" class="form-control form-control-sm" id="Kamar" value="<?= $data_cppt["kamar"]; ?>"/>
          </div>
          <div class="col-md-3">
            ...
            <select name="namadokter" id="namadokter" class="form-select form-select-sm">
              ...
              while ($data = mysqli_fetch_array($sql)) {
                echo "<option " . ($data_cppt["nama_dokter"] == $data["nama_dokter"] ? "selected" : "") . " value='$data[nama_dokter]'>$data[nama_dokter]</option>";
              }
              ?>
            </select>
          </div>
  ...
  <textarea class="form-control" name="cppt" id="cppt" rows="3"><?= isset($data_cppt["hasil"]) ? htmlspecialchars($data_cppt["hasil"]) : ''; ?></textarea>
</div>
