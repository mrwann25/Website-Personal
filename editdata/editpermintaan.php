...
if (!isset($_GET["id"])) {
  ...
  $form_permintaan = query("SELECT * FROM db_permintaanalat WHERE id = $permintaan");

  $TotalData = count($form_permintaan);

  if ($TotalData == 0) {
    echo "ID tidak ditemukan";
    exit;
  } else {
    $form_permintaan = query("SELECT * FROM db_permintaanalat WHERE id = $permintaan")[0];
  }
}
...
<head>
  ...
  <title>Permintaan Alat Masak</title>
</head>
 <!-- css footer -->
 <style>
    .footer {
...
<body>
  ...
  <div class="container mt-1">
    <h4 class="text-center">Form Permintaan Alat (Edit Data)</h4>
    <div style="background-color: rgb(228, 228, 228)">
      <div class="container-fluid">
        <form action="" method="post" class="row g-3 mt-2">
          <input type="hidden" name="id" value="<?= $form_permintaan["id"]; ?>">
          <div class="col-md-6">
            <label for="namabarang" class="form-label">Nama Barang</label>
            <input type="text" class="form-control" id="namabarang" name="namabarang" required value="<?= $form_permintaan["nama_alat"]; ?>" />
          </div>
          <div class="col-md-6">
            <label for="jumlahpermintaan" class="form-label">Jumlah Permintaan</label>
            <input type="text" class="form-control" id="jumlahpermintaan" name="jumlahpermintaan" required value="<?= $form_permintaan["jumlah_permintaan"]; ?>" />
          </div>
          <div class="col-md-6">
            <label for="jumlahdiberi" class="form-label">Jumlah Yang Diberi</label>
            <input type="text" class="form-control" id="jumlahdiberi" name="jumlahdiberi" required value="<?= $form_permintaan["jumlah_diberi"]; ?>" />
          </div>

          <div class="col-md-6">
            <label for="keterangan" class="form-label">Keterangan</label>
            <input type="text" class="form-control" id="keterangan" name="keterangan" required value="<?= $form_permintaan["keterangan"]; ?>" />
          </div>
          <div class="col-md-6">
            <label for="namapetugas" class="form-label">Nama Petugas</label>
            <input type="text" class="form-control" id="namapetugas" name="namapetugas" required value="<?= $form_permintaan["nama_petugas"]; ?>" />
          </div>
          <div class="col-md-6">
            <label for="yangmenyetujui" class="form-label">Yang Menyetujui</label>
            <input type="text" class="form-control" id="yangmenyetujui" name="yangmenyetujui" required value="<?= $form_permintaan["nama_pejabat"]; ?>" />
          </div>
          <div class="col-md-6">
            <label for="tanggal" class="form-label">Tanggal</label>
            <input type="date" class="form-control form-control-sm-2" id="tanggal" name="tanggal" required value="<?= $form_permintaan["tanggal"]; ?>" />
          </div>
          <div class="d-flex gap-2 d-md-relative justify-content-md-end">
            <button type="submit" name="submit" class="btn btn-warning">Submit</button>
            <a href="../Form_Rekap_Data/permintaanalat.php" class="btn btn-sm btn-secondary" style="width: 150px;">Back</a>
          </div>
        </form>
      </div>
    </div>
